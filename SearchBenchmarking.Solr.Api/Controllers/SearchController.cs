...
using SearchBenchmarking.Library.DTOs; // Tilgå SearchRequest og SearchResult
using SearchBenchmarking.Library.Interfaces; // Tilgå ISearchService
using System;
...
namespace SearchBenchmarking.Solr.Api.Controllers
{
    ...
    public class SearchController : ControllerBase
    {
        ...
        ///
        /// </remarks>
        [HttpGet] // Håndterer HTTP GET requests til /api/Search
        [ProducesResponseType(typeof(SearchResult), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Search([FromQuery] SearchRequest request)
        {
            // [FromQuery] binder URL query parametre (Query, PageSize, StartFrom) til SearchRequest objektet.
            // Dette forudsætter, at SearchRequest DTO'en har simple properties, som model-binderen kan håndtere.
            ...
            {
                // request.PageSize = 50; // Sæt en default eller returner BadRequest
            ...
            try
            {
                _logger.LogInformation("Received search request for Solr: Query='{Query}', PageSize={PageSize}, StartFrom={StartFrom}",
                ...
                if (result == null)
                {
                    ...
                    _logger.LogWarning("Search service returned null for Solr query: {Query}", request.Query);
                    return Ok(new SearchResult()); // Returner et tomt, men gyldigt, resultat
                }

                _logger.LogInformation("Solr search successful for Query='{Query}'. Found {TotalHits} hits. Returning {IdCount} IDs.",
            ...
            {
                _logger.LogWarning(argEx, "Invalid argument for Solr search request: {Query}", request.Query);
                return BadRequest(new ProblemDetails { Title = "Invalid search parameters.", Detail = argEx.Message, Status = StatusCodes.Status400BadRequest });
            }
            catch (Exception ex)
            {
                // Log den fulde exception for debugging
                _logger.LogError(ex, "An unexpected error occurred during Solr search for query: {Query}", request.Query);
